# 🚀 Awesome Developer Resources

> **Created by Antono**


> A curated collection of the best resources, tools, and links for developers to boost productivity and learn new skills.

<!-- Badges -->
<div align="center">

![Stars](https://img.shields.io/github/stars/antono4/awesome-developer-resources?style=for-the-badge)
![Forks](https://img.shields.io/github/forks/antono4/awesome-developer-resources?style=for-the-badge)
![Issues](https://img.shields.io/github/issues/antono4/awesome-developer-resources?style=for-the-badge)
![Contributors](https://img.shields.io/github/contributors/antono4/awesome-developer-resources?style=for-the-badge)
![Last Commit](https://img.shields.io/github/last-commit/antono4/awesome-developer-resources/main?style=for-the-badge)

**The ultimate curated collection of developer resources for learning, building, and growing.**

[![Stars](https://img.shields.io/github/stars/antono4/awesome-developer-resources?style=social)](https://github.com/antono4/awesome-developer-resources/stargazers)
[![Forks](https://img.shields.io/github/forks/antono4/awesome-developer-resources?style=social)](https://github.com/antono4/awesome-developer-resources/network/members)
[![Contributors](https://img.shields.io/github/contributors/antono4/awesome-developer-resources)](https://github.com/antono4/awesome-developer-resources/graphs/contributors)

A curated collection of awesome developer resources, tools, libraries, and learning materials to help developers build better software faster.

## 📚 Contents

- [Learning Platforms](#-learning-platforms)
- [Free APIs](#-free-apis)
- [Developer Tools](#-developer-tools)
- [Code Editors & IDEs](#-code-editors--ides)
- [Design Resources](#-design-resources)
- [Hosting & Deployment](#-hosting--deployment)
- [APIs](#-apis)
- [Cheat Sheets](#-cheat-sheets)
- [Community](#-community)

---

## 🎓 Learning Platforms

| Platform | Description | Free? |
|----------|-------------|-------|
| [freeCodeCamp](https://freecodecamp.org) | Learn to code for free | ✅ |
| [The Odin Project](https://www.theodinproject.com) | Full-stack curriculum | ✅ |
| [Exercism](https://exercism.org) | Code exercises with mentoring | ✅ |
| [Codecademy](https://codecademy.com) | Interactive coding lessons | 🟡 |
| [Coursera](https://coursera.org) | University courses online | 🟡 |
| [MDN Web Docs](https://developer.mozilla.org) | Web documentation | ✅ |

---

## 🌐 Free APIs

| API | Description | Rate Limit |
|-----|-------------|------------|
| [JSONPlaceholder](https://jsonplaceholder.typicode.com) | Fake data for testing | Unlimited |
| [REST Countries](https://restcountries.com) | Country information | 15k/month |
| [The Dog API](https://thedogapi.com) | Dog images and facts | 100/day |
| [The Cat API](https://thecatapi.com) | Cat images and facts | 100/hour |
| [Open Library API](https://openlibrary.org/developers/api) | Book data | - |
| [Public APIs](https://github.com/public-apis/public-apis) | Collection of free APIs | Varies |

---

## 🛠️ Developer Tools

### IDEs & Code Editors
- [VS Code](https://code.visualstudio.com) - Powerful, extensible code editor
- [Cursor](https://cursor.sh) - AI-first code editor
- [Vim](https://www.vim.org) - Classic modal editor
- [Neovim](https://neovim.io) - Hyperextensible Vim editor
- [Sublime Text](https://sublimetext.com) - Fast code editor
- [JetBrains IDEs](https://jetbrains.com) - Professional IDEs

### Terminal Tools
- [Oh My Zsh](https://ohmyz.sh) - Framework for Zsh
- [Starship](https://starship.rs) - Cross-shell prompt
- [fzf](https://github.com/junegunn/fzf) - Fuzzy finder
- [tmux](https://tmux.github.io) - Terminal multiplexer
- [Fish Shell](https://fishshell.com) - Smart, user-friendly shell

### Productivity
- [Notion](https://notion.so) - Notes and workspace
- [Obsidian](https://obsidian.md) - Knowledge base
- [Linear](https://linear.app) - Issue tracking
- [Raycast](https://raycast.com) - Spotlight alternative

---

## 💻 Code Editors & IDEs

### Online IDEs
| Editor | Description |
|--------|-------------|
| [CodePen](https://codepen.io) | Front-end playground |
| [JSFiddle](https://jsfiddle.net) | JavaScript playground |
| [Replit](https://replit.com) | Online IDE |
| [StackBlitz](https://stackblitz.com) | Instant dev environments |
| [GitHub Codespaces](https://github.com/features/codespaces) | Cloud IDE |
| [VS Code Online](https://vscode.dev) | Browser VS Code |

---

## 🎨 Design Resources

### UI Components
- [Tailwind CSS](https://tailwindcss.com) - Utility-first CSS
- [Shadcn/ui](https://ui.shadcn.com) - Beautifully designed components
- [Radix UI](https://radix-ui.com) - Unstyled, accessible components
- [Headless UI](https://headlessui.com) - Invisible UI components
- [Flowbite](https://flowbite.com) - Tailwind components

### Icons
- [Lucide](https://lucide.dev) - Beautiful open source icons
- [Phosphor Icons](https://phosphoricons.com) - Flexible icon family
- [Heroicons](https://heroicons.com) - Tailwind CSS icons
- [Tabler Icons](https://tabler-icons.io) - SVG icons
- [Iconify](https://iconify.design) - Universal icon framework

### Images & Assets
- [Unsplash](https://unsplash.com) - Free photos
- [Pexels](https://pexels.com) - Free stock photos
- [Lorem Picsum](https://picsum.photos) - Random placeholder images
- [RemoveBG](https://remove.bg) - Remove image backgrounds

---

## ☁️ Hosting & Deployment

| Service | Description | Free Tier |
|---------|-------------|-----------|
| [Vercel](https://vercel.com) | Front-end deployment | ✅ |
| [Netlify](https://netlify.com) | Static site hosting | ✅ |
| [GitHub Pages](https://pages.github.com) | Free static hosting | ✅ |
| [Cloudflare Pages](https://pages.cloudflare.com) | Fast global CDN | ✅ |
| [Railway](https://railway.app) | Infrastructure | ✅ |
| [Render](https://render.com) | Cloud hosting | ✅ |
| [Fly.io](https://fly.io) | Edge computing | ✅ |

---

## 📡 APIs

### AI & ML
| API | Description |
|-----|-------------|
| [OpenAI API](https://openai.com/api) | GPT models |
| [Anthropic](https://anthropic.com) | Claude AI |
| [Google AI](https://ai.google.dev) | Gemini, PaLM |
| [Hugging Face](https://huggingface.co) | Open source AI |

### Maps
| API | Description |
|-----|-------------|
| [Mapbox](https://mapbox.com) | Maps and location |
| [Leaflet](https://leafletjs.com) | Open source maps |
| [Google Maps](https://developers.google.com/maps) | Google Maps API |

### More APIs
| API | Description |
|-----|-------------|
| [Stripe](https://stripe.com) | Payment processing |
| [SendGrid](https://sendgrid.com) | Email service |
| [Twilio](https://twilio.com) | Communications |
| [Firebase](https://firebase.google.com) | Google's platform |

---

## 📋 Cheat Sheets

- [Git Cheat Sheet](https://education.github.com/git-cheat-sheet-education.pdf)
- [CSS Cheat Sheet](https://quickref.me/css)
- [JavaScript Cheat Sheet](https://quickref.me/javascript)
- [Python Cheat Sheet](https://quickref.me/python)
- [React Cheat Sheet](https://quickref.me/react)
- [Docker Cheat Sheet](https://quickref.me/docker)
- [Linux Cheat Sheet](https://quickref.me/linux)
- [Vim Cheat Sheet](https://quickref.me/vim)

---

## 👥 Community

### Forums & Discussion
- [Stack Overflow](https://stackoverflow.com) - Q&A for developers
- [Reddit](https://reddit.com/r/programming) - Programming discussions
- [DEV Community](https://dev.to) - Developer community
- [Hashnode](https://hashnode.com) - Blogging for developers
- [GitHub Discussions](https://github.com/features/discussions) - Repository discussions

### News & Updates
- [Hacker News](https://news.ycombinator.com) - Tech news
- [Product Hunt](https://producthunt.com) - New products
- [TechCrunch](https://techcrunch.com) - Tech news
- [The Verge](https://theverge.com) - Technology

### YouTube Channels
- [Traversy Media](https://youtube.com/@TraversyMedia) - Web development
- [Fireship](https://youtube.com/@Fireship) - Quick code lessons
- [Web Dev Simplified](https://youtube.com/@WebDevSimplified) - Simplified tutorials
- [The Net Ninja](https://youtube.com/@NetNinja) - Web tutorials
- [Clément Mihailescu](https://youtube.com/@CleverProgrammer) - Coding bootcamp

---

## 🤝 Contributing

Contributions are welcome! Please feel free to submit a Pull Request.

1. Fork the repository
2. Create your feature branch (`git checkout -b feature/AmazingFeature`)
3. Commit your changes (`git commit -m 'Add some AmazingFeature'`)
4. Push to the branch (`git push origin feature/AmazingFeature`)
5. Open a Pull Request

---

## 📄 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

---

## ⭐ Show Your Support

Give a ⭐ if this project helped you!

[![Stars](https://img.shields.io/github/stars/antono4/awesome-developer-resources?style=social)](https://github.com/antono4/awesome-developer-resources/stargazers)

---

<p align="center">
  Made with ❤️ by <a href="https://github.com/antono4">antono4</a>
</p>
